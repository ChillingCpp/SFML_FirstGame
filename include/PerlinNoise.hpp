...
public:
    ...
    {
        int h = hash & 15;
        double u = h < 8 ? x : 0, v = h < 4 ? 0 : h == 12 || h == 14 ? x : 0;
    ...
    double noise(double x)
    {
        ...
        double u = fade(x), v = fade(0), w = fade(0);
        int A = int(p[X]), AA = int(p[A]), AB = int(p[A + 1]),
            B = int(p[X + 1]), BA = int(p[B]), BB = int(p[B + 1]);

        return lerp(
            w,
            lerp(v,
                 lerp(u, grad((int)p[AA], x, 0, 0), grad((int)p[BA], x - 1, 0, 0)),
                 lerp(u, grad((int)p[AB], x, - 1, 0),
                      grad((int)p[BB], x - 1, - 1, 0))),
            lerp(v,
                 lerp(u, grad((int)p[AA + 1], x, 0, - 1),
                      grad((int)p[BA + 1], x - 1, 0, - 1)),
                 lerp(u, grad((int)p[AB + 1], x, - 1,  - 1),
                      grad((int)p[BB + 1], x - 1, - 1, - 1))));
    }
private:
    std::array<int, 512> p{
