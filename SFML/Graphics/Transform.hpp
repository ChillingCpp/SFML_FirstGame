...
#include <SFML/Graphics/Transform22.hpp>
...
public:
    ...
    Vector3f transformPoint3(float x, float y, float z) const{
        return Vector3f(
            m_matrix[0] * x + m_matrix[4] * y + m_matrix[12] * z,
            m_matrix[1] * x + m_matrix[5] * y + m_matrix[13] * z,
            m_matrix[3] * x + m_matrix[7] * y + m_matrix[15] * z);
    }
    Vector3f transformPoint3(const Vector3f& point) const{
        return transformPoint3(point.x, point.y, point.z);
    }
    ...
    Vector2f Solve(const Vector2f &b){
        Transform transform = getInverse();
        return transform.transformPoint(b);
    }
    Vector3f Solve33(const Vector3f &b){

        Transform transform = getInverse();
        return transform.transformPoint3(b);
    }
    Vector2f Solve22(const sf::Vector2f &b){
        Transform22 transform(m_matrix[0], m_matrix[1], m_matrix[4], m_matrix[5]);
        Transform22 inverse = transform.getInverse(); 
        return inverse.transformPoint(b); 
    }
